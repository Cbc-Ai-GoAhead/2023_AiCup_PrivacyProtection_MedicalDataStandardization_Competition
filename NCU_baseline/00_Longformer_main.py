...
if __name__ == '__main__':
  ...
  writer = SummaryWriter(comment='ncu_Lonformer')
  ...
  for epoch in range(EPOCH):
    ...
    if sum_val_F1 > base_f1_score:
      ...
      if not os.path.exists(exp_path):
        os.mkdir(exp_path)
      output_dir = "./model_longformer/" + "best_longgormer-base-4096"+"_"+str(epoch)+"_"+str(BACH_SIZE)+"_"+str(sum_val_F1/len(val_dataloader))
      # model.save_pretrained(output_dir)
      # output_dir= output_dir+"_token"
      # tokenizer.save_pretrained(output_dir)
      # torch.save(model.state_dict(), output_dir+"dict")
  ...
  exp_path = "./submission_Longformer"+"_"+str(epoch)+"_"+str(BACH_SIZE)
  if not os.path.exists(exp_path):
      os.mkdir(exp_path)
